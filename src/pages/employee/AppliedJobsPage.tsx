...
const AppliedJobsPage: FC = () => {
  ...
  const onFilter = (e: React.ChangeEvent<HTMLSelectElement | any>) => {
    ...
    if (["1", "2", "3"].includes(status.toString())) {
      let filtered_jobs2 = filtered_jobs.filter(
        (job: any) => job.applicant.status === status,
      );
  ...
  const onClearFilter: React.MouseEventHandler<
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-12 col-md-12 col-xs-12">
              ...
              {!loading && filtered_jobs.length === 0 && <h2>No jobs found</h2>}
              {!loading &&
                    ...
                    >
                      <div className="row">
                        ...
                        <div className="col-lg-2 col-md-2 col-xs-12 text-center">
                          {job.filled && (
                            <span className="btn-open">Position filled</span>
                          )}
                          {!job.filled && (
                            <span className="btn-open">Position open</span>
                          )}
                        </div>
                        ...
                        <div className="col-lg-2 col-md-2 col-xs-12 text-right">
                          ...
                          {job.type === "2" && (
                            <span className="btn-full-time">Part Time</span>
                          )}
                          {job.type === "3" && (
                            <span className="btn-full-time">Internship</span>
                          )}
                        </div>
                      </div>
                    </Link>
