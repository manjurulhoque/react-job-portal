(function (global, factory) {
	...
	var toolbar = renderer.create(
		'<div class="note-toolbar-wrapper panel-default"><div class="note-toolbar panel-heading"></div></div>',
	);
	...
	$$1.summernote = $$1.summernote || { lang: {} };
		...
		WrappedRange.prototype.nativeRange = function () {
			if (env.isW3CRangeSupport) {
				...
				textRange.setEndPoint(
					"EndToEnd",
					pointToTextRange({ node: this.ec, offset: this.eo }),
				);
				return textRange;
			}
		};
	...
	var Style = (function () {
		...
		Style.prototype.stylePara = function (rng, styleInfo) {
			$$1.each(
				rng.nodes(dom.isPara, { includeAncestor: true }),
				function (idx, para) {
		...
		Style.prototype.styleNodes = function (rng, options) {
			...
			var nodes = rng
				.nodes(dom.isText, { fullyContains: true })
				.map(function (text) {
					return (
	...
	var Bullet = (function () {
		...
		Bullet.prototype.toggleList = function (listName, editable) {
			...
			if (lists.find(paras, dom.isPurePara)) {
				...
				var diffLists = rng
					.nodes(dom.isList, { includeAncestor: true })
		...
		Bullet.prototype.releaseList = function (clustereds, isEscapseToBody) {
			...
			$$1.each(clustereds, function (idx, paras) {
				...
				var middleList = dom.splitTree(
					headList,
					{ node: head.parentNode, offset: dom.position(head) },
					{ isSkipPaddingBlankHTML: true },
				);
	...
	var Buttons = (function () {
		...
		Buttons.prototype.build = function ($container, groups) {
			for (
				...
				var buttons = $$1.isArray(group)
					...
					: [group];
				var $group = this.ui
					.buttonGroup({ className: "note-" + groupName })
					.render();
		...
		Buttons.prototype.tableMoveHandler = function (event) {
			var PX_PER_EM = 18;
			var $picker = $$1(event.target.parentNode);
		...
		HelpDialog.prototype.initialize = function () {
			...
			this.$dialog = this.ui
				.dialog({
					...
					callback: function ($node) {
						$node
							.find(".modal-body,.note-modal-body")
